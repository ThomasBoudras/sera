...
from src.models.networks.regression.components.ltae import LTAE2d


class UTAES(nn.Module):
    def __init__(
        self,
        input_dim,
    ...
    def forward(self, input, meta_data=None, return_att=False):
        ...
        batch_positions_seperation = batch_positions.shape[1]//2

        input_1 = input[:, :input_separation,:, :, :].contiguous()
        batch_positions_1 = batch_positions[:,:batch_positions_seperation].contiguous()

        input_2 = input[:, input_separation:, :, :, :].contiguous()
        batch_positions_2 = batch_positions[:, batch_positions_seperation:].contiguous()
        ...
        for i in range(self.n_stages - 1):
            ...
            out = self.up_blocks_samiese[i](input=out, skip_1=skip_1, skip_2=skip_2)
...
class UpConvBlock_siamese(nn.Module):
    ...
    def forward(self, input, skip_1, skip_2):
        ...
        if self.coupling_mode == "difference" :
            ...
            out_1 = out[:, :out_separation,:, :].contiguous()
            out_2 = out[:, out_separation:, :, :].contiguous()
            out = torch.cat([out_1, self.skip_conv(skip_1), out_2, self.skip_conv(skip_2)], dim=1)

        else :
            Exception(f"Give a format for coupling mode valid: either “difference” or “concat”.")
        out = self.conv1(out)
        out = out + self.conv2(out)
        return out
