...
class Module(LightningModule):
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        number_geometry = meta_data["number_geometry"]
        
        if self.predictions_save_dir is not None:
            np.save(
                os.path.join(self.predictions_save_dir, "pred", str(batch_idx)  + ".npy"),
                res.cpu().numpy().astype(np.float32),
            )
            np.save(
                os.path.join(self.predictions_save_dir, "input", str(batch_idx)  + ".npy"),
                input.cpu().numpy().astype(np.float32),
            )
            np.save(
                os.path.join(self.predictions_save_dir, "bounds", str(batch_idx)  + ".npy"),
                bounds.cpu().numpy().astype(np.float32),
            )
            np.save(
                os.path.join(self.predictions_save_dir, "number_geometry", str(batch_idx)  + ".npy"),
                number_geometry.cpu().numpy().astype(np.float32),
            )
            if self.save_target : 
                target = crop_tensor(target)
                np.save(
                    os.path.join(self.predictions_save_dir, "target", str(batch_idx)  + ".npy"),
